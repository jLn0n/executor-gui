...
local function updateIDE()
	...
	TextboxInput.Text = sanitizeText(TextboxInput.Text, "tabs") -- sanitize cringe \t
...
SidebarToggleBtn.MouseButton1Click:Connect(function()
	...
	local sidebarTween, fadeTween, sidebarBtnTween

	if sidebarOpen then
		sidebarTween, fadeTween = setSidebarVisible(false)
		sidebarBtnTween = setSdBtnRot(0)
	else
		sidebarTween, fadeTween = setSidebarVisible(true)
		sidebarBtnTween = setSdBtnRot(90)
	end
...
inputService.InputBegan:Connect(function(input, gameProcessed)
	if input.KeyCode == Enum.KeyCode.Equals and not inputService:GetFocusedTextBox() then
		...
		local toggleTween = toggleUI(MainUI.Visible)
